...
public class DynamicMippedTexture extends AbstractTexture implements Dumpable {
    ...
    private DynamicMippedTexture(final int mipCount) {
        ...
        this.mipPixels = new NativeImage[mipCount];
        this.mipViews = new GpuTextureView[mipCount];
    }

    public DynamicMippedTexture(final String label, final int width, final int height, final boolean zero, final int mipCount) {
        this(mipCount);
        final GpuDevice device = RenderSystem.getDevice();
        this.texture = device.createTexture(label, 5, TextureFormat.RGBA8, width, height, 1, this.mipCount);
        this.createTexture(device, zero);
    }

    public DynamicMippedTexture(final Supplier<String> label, final int width, final int height, final boolean zero, final int mipCount) {
        this(mipCount);
        final GpuDevice device = RenderSystem.getDevice();
        this.texture = device.createTexture(label, 5, TextureFormat.RGBA8, width, height, 1, this.mipCount);
        this.createTexture(device, zero);
