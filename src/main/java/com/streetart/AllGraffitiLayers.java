...
//            (p, _) -> p.getItemBySlot(EquipmentSlot.HEAD).is(AllItems.SWAG)
    ...
    public static GraffitiLayerType getActive(final Player player, final Level level) {
        GraffitiLayerType highest = null;
        for (final GraffitiLayerType graffitiLayerType : LAYER_REGISTRY) {
    ...
    private static GraffitiLayerType register(final String id, final int renderingPriority, final BiPredicate<Player, Level> visibility) {
